...
- **浏览与搜索** — 技能卡片列表，支持关键词搜索和标签分类过滤
- **上传技能** — 支持选择文件夹（整目录上传）、拖拽文件夹/文件、自动识别 `skill.json` 填充元数据
- **下载技能** — `.tar.gz` 格式打包下载，保留完整目录结构
- **技能详情** — 查看基本信息、包含文件列表、README 内容
- **删除技能** — 从市场中移除技能包
...
│   │   ├── marketApi.ts           # 技能市场 API 客户端
│   │   └── types.ts               # TypeScript 类型定义
│   └── components/
│       ├── Sidebar.tsx            # 左侧导航栏（对话 / 管理面板 / 技能 / 市场）
│       ├── ChatView.tsx           # 主聊天界面
...
│       ├── SkillMarketPanel.tsx   # 技能市场主面板（搜索/过滤/列表/详情）
│       ├── SkillCard.tsx          # 技能卡片组件
│       └── SkillUploadDialog.tsx  # 技能上传对话框（支持文件夹上传/拖拽）
├── server.mjs                     # 生产服务器（静态文件 + WS 代理 + 文件上传 + 技能市场 API + Turn 缓存）
├── oss.mjs                        # 阿里云 OSS 工具模块（零依赖，原生 crypto + https）
├── env.mjs                        # .env 文件加载器
├── .env.example                   # 环境变量模板
...
| 大文件 (>512KB) | 配置 OSS 时上传到阿里云 OSS，返回签名 URL（24h 有效）；未配置时上传到本地 `uploads/` 目录 |

> 大文件上传模式需要 Agent 的 tools profile 设为 `coding` 或 `full`。
> - **OSS 模式**：Agent 通过签名 URL 直接下载文件（支持跨服务器部署）
> - **本地模式**：Agent 通过文件路径读取（要求 WebChat 与 Gateway 同服务器）
...
cp -r dist server.mjs oss.mjs env.mjs package.json deploy/
...
- [ ] 打包 `dist/` + `server.mjs` + `oss.mjs` + `env.mjs` + `package.json` + `node_modules/ws`
- [ ] 传输到服务器
- [ ] 确认 Node.js 18+
- [ ] `node server.mjs`
- [ ] 访问 `http://服务器IP:5200`
...
### 技能市场与 OpenClaw 联动（下一步重点）

当前技能市场支持上传/下载/管理技能包，但尚未与 OpenClaw 后台直接联动。计划：

- [ ] **一键安装到 OpenClaw** — 在 OpenClaw 所在服务器部署轻量 agent 脚本，从 OSS 拉取技能包并自动注册到 `managedSkillsDir`
- [ ] **集成 ClawHub API** — 从 OpenClaw 官方技能市场（clawhub.ai）搜索、浏览、一键安装
- [ ] **自定义 Registry** — 利用 ClawHub 的 `--registry` 机制，支持搭建私有技能仓库
- [ ] **技能版本管理** — semver 版本号、更新检查、版本历史

### 其他

- [ ] Turn 缓存迁移至 OSS 或 Redis（当前仍为本地 `data/turns/`）
...
| 存储模块 | 配置 OSS 时 | 未配置 OSS 时 |
|---------|------------|-------------|
| 对话附件 | OSS `uploads/` + 签名 URL | 本地 `uploads/` + 绝对路径 |
| 技能包 | OSS `skills/packages/` | 本地 `data/skills/packages/` |
| 技能注册表 | OSS `skills/registry.json` | 本地 `data/skills/registry.json` |
| Turn 缓存 | 本地 `data/turns/`（暂未迁移） | 本地 `data/turns/` |
...
- **技能市场** — 新增独立的技能市场模块（侧边栏"市场"tab），支持浏览、搜索、标签过滤、上传、下载（.tar.gz）、查看详情、删除
- **文件夹上传** — 技能上传支持选择文件夹（webkitdirectory）、拖拽文件夹/文件，自动识别 `skill.json` 填充元数据
- **阿里云 OSS 集成** — 零依赖原生实现（`oss.mjs`），对话附件和技能包存储自动切换 OSS/本地模式
- **签名 URL** — Private Bucket 模式下上传文件返回 24h 有效签名 URL，Agent 可跨服务器直接下载
- **`.env` 配置** — 新增 `env.mjs` 加载器 + `.env.example` 模板，密钥通过环境变量管理
- **MIME 修复** — 修复 `.mjs` 文件 MIME 类型缺失导致的 module script 加载失败
