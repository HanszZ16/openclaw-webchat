...
- **Markdown 渲染** — 代码高亮、表格、列表、LaTeX 等完整渲染
- **Tool 调用展示** — 可折叠查看 Agent 工具调用的参数和结果
- **文件上传** — 支持图片、PDF、Word、Excel、代码等文件，自动智能分流处理
...
访问 `http://localhost:5173`，在登录页输入 Gateway 地址（如 `ws://127.0.0.1:18789`）和密码。
...
│       ├── ToolCard.tsx           # Tool 调用展示卡片
│       ├── ModelSelector.tsx      # 模型选择
│       ├── LoginPage.tsx          # 登录页
│       ├── AdminPanel.tsx         # 管理面板（系统状态/团队/会话/日志/定时任务）
│       └── SkillsPanel.tsx        # 已安装技能展示
├── server.mjs                     # 生产服务器（静态文件 + WS 代理 + 文件上传）
├── vite.config.ts                 # Vite 开发配置（含 WS 代理插件）
├── index.html
├── package.json
├── tsconfig.json
└── .gitignore
...
| 方法 | 用途 | 面板 |
|------|------|------|
| `connect` | 建立连接、认证、获取 snapshot | 全局 |
| `chat.send` | 发送消息 | 对话 |
